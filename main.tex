...
\author{
  \begin{minipage}{\linewidth}
    \centering
    \authoritem{Menghua Zhai}{ted@cs.uky.edu}{1}
    \authoritem{Tawfiq Salem}{salem@cs.uky.edu}{1}
    %\authoritem{Connor Greenwell}{connor@cs.uky.edu}{1}
    %\authoritem{Scott Workman}{scott@cs.uky.edu}{1}
    %\authoritem{Robert Pless}{pless@gwu.edu}{2}
    %\authoritem{Nathan Jacobs}{jacobs@cs.uky.edu}{1}
    \\[.2cm]
    \hspace{-14pt}
...
\runninghead{Zhai}{Learning Geo-Temporal Image Features}
...
%Time annotation from both data resources are recorded automatically by
%the devices. The Flickr images are mostly captured by cell phones
%therefore they are tagged with GPS locations.  The only targets we
%used for training that require manual annotation are the geographic
%location of the webcams, which are registered and uploaded by users.
...
%We start by analyzing the correspondences of the learned mid-level
%features with the image appearance changes, then we train the our
%network on various datasets.  We offer visualizations of the meanings
%of specific features in the image context networks, and show that the
%image context features have strong correlations with transient image
%conditions. We further show our method is a good pre-training strategy
%for learning the transient attributes.  As applications, we show that
%our model can be used for geolocalization and time estimation.
...
\begin{figure}
  ...
  \begin{tabular}{lcc}
    ...
    %\includegraphics[width=.45\linewidth]{webcam_localization/8260-loss_maps_amos-5} &
